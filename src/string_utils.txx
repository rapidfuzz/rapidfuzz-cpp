...
string_utils::join(const string_view_vec<CharT>& sentence)
{
  if (sentence.empty()) {
 ...
 */
template <typename CharT1, typename CharT2>
Affix string_utils::remove_common_affix(basic_string_view<CharT1>& a,
                                        basic_string_view<CharT2>& b)
{
  return Affix{remove_common_prefix(a, b), remove_common_suffix(a, b)};
}
...
string_utils::char_freq(const std::vector<Sentence>& str)
{
  std::array<unsigned int, 32> char_freq{};
...
std::array<unsigned int, 32>
string_utils::char_freq(const std::array<unsigned int, 32>& freq)
{
  return freq;
}

template <typename Sentence1, typename Sentence2>
std::size_t string_utils::count_uncommon_chars(const Sentence1& s1,
                                               const Sentence2& s2)
{
...
template <typename CharT>
std::basic_string<CharT>
string_utils::default_process(std::basic_string<CharT> s)
{
  replace_non_alnum(s);
