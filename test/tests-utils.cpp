...
TEST_CASE("to_string_view")
{
    ...
    {
		auto str_view = utils::to_string_view(str);
        REQUIRE(str.data() == str_view.data());
		REQUIRE(str.size() == str_view.size());

		auto wstr_view = utils::to_string_view(wstr);
