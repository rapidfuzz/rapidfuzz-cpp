...
std::vector<EditOp> edit_ops_from_cost_matrix(basic_string_view<CharT1> s1, basic_string_view<CharT2> s2, std::size_t prefix_len,
                                                  std::vector<std::size_t> matrix)
{
    ...
    while (row > 0 || column > 0) {
      ...
      if (row != 0 && column != 0 && matrix[current_pos] == matrix[diagonal_pos] && s1[row - 1] == s2[column - 1]) {
        --row;
        --column;
...
std::vector<MatchingBlock> get_matching_blocks(const Sentence1& sentence1, const Sentence2& sentence2) {
