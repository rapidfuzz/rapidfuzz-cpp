...
#
# This script converts Unicode database files to rapidfuzz/details/unicode.cpp,
#
# history:
...
# 2021-11    maxbachmann modified to generate mappings required in rapidfuzz
#
...
def makeunicodetype(unicode, trace):
    ...
    FILE = "rapidfuzz/details/unicode.cpp"
    ...
    with open(FILE, "w") as fp:
        ...
        fprint()
        fprint("#include <rapidfuzz/details/unicode.hpp>")
        fprint()
        fprint("namespace rapidfuzz {")
        fprint("namespace Unicode {")
        fprint("namespace detail {")
        fprint()
        fprint("/* a list of unique character type descriptors */")
        fprint("const TypeRecord TypeRecords[] = {")
        for item in table:
        ...
        fprint()
        fprint("const uint32_t ExtendedCase[] = {")
        ...
        # Generate code for IsWhitespace_u8()
        fprint("/* Returns 1 for Unicode characters having the bidirectional")
        fprint(" * type 'WS', 'B' or 'S' or the category 'Zs', 0 otherwise.")
        fprint(" */")
        fprint('bool IsWhitespace_u8(const uint8_t ch)')
        fprint('{')
        fprint('    switch (ch) {')
